...
echo Hytale Generator Workbench v0.11.36-rc.4-r1 - Dependency Lock Capture
...
cargo tree --locked --manifest-path src-tauri\Cargo.toml > build\cargo-tree-v0.11.36-rc.4.txt
if errorlevel 1 goto :failed
cargo metadata --locked --format-version 1 --manifest-path src-tauri\Cargo.toml > build\cargo-metadata-v0.11.36-rc.4.json
if errorlevel 1 goto :failed
...
echo   v0.11.36-rc.4-r1 keeps that exact verified lock as local build evidence before a release build
