...
function importSpecifiers(source) {
  ...
  for (const matcher of [staticImport, dynamicImport]) {
    matcher.lastIndex = 0;
    for (let match = matcher.exec(source); match; match = matcher.exec(source)) values.add(match[1]);
  }
  return [...values];
}
...
assert.deepEqual(cycles, [], `frontend relative-import cycles detected:\n${cycles.map((cycle) => cycle.join(' -> ')).join('\n')}`);
...
console.log(JSON.stringify({
  ...
  relativeEdges: [...graph.values()].reduce((sum, deps) => sum + deps.length, 0),
