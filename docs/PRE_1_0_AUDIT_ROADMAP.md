...
| 10 | I/O, ZIP, import/export & large-project safety | Folder loading, output policy, ZIP32 limits, binary copies, semantic discovery, previews | Boundary/overflow fixtures, explicit errors instead of silent truncation, representative large-project run | **IN REVIEW** |
...
Track 10 is in review in `docs/audits/PRE_1_0_10_IO_ZIP_EXPORT.md`, with `test:pre1-io-zip-export` and Windows-native I/O fixtures. Full regression and CI validation are pending.
