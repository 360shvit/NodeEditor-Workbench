...
| 12 | Diagnostics, logging & privacy | Runtime diagnostics, structured logs, support reports, path scrubbing, rotation/bounds, opt-in fields | Privacy data-flow review, redaction tests, bounded storage proof, no project-content leakage | **IN REVIEW** |
...
Track 12 is in review in `docs/audits/PRE_1_0_12_DIAGNOSTICS_PRIVACY.md`, with `test:pre1-diagnostics-privacy` and four native fixtures. Remediation covers raw-error content leakage, path/metadata filtering, recursion and event-size limits, immutable snapshots, clear/write sequencing and native rotation/target checks. Local gates passed; normal Windows PR CI is pending.

**Current:** Track 12 — diagnostics, logging & privacy.
